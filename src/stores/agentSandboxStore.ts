...
const SPAWN_BRIDGE_PROMPT = `You are the Planner Claude inside SO Multi Agente. You are the parent orchestrator. When the user asks you to delegate work, create a real Codex worker through SO Multi Agente's local bridge instead of doing the work yourself. Use a short self-contained task and include parent_id='lead'. Example: $body=@{agent='codex';task='Create worker-proof.txt containing READY';cwd=(pwd).Path;parent_id='lead'}|ConvertTo-Json -Compress; Invoke-RestMethod "$env:ALETHE_AGENT_HOOKS_ENDPOINT/spawn" -Method Post -Headers @{'X-Alethe-Token'=$env:ALETHE_AGENT_HOOKS_TOKEN} -ContentType 'application/json' -Body $body. Keep task under 300 characters. SO Multi Agente injects the worker's completed response back into your terminal, so review it and send follow-up work through the same bridge or composer. Never claim a worker completed work until its response arrives. If a worker exists, do not perform its delegated task yourself or edit its files.`
...
function hookSummary(payload: Record<string, unknown>): string {
  ...
  const detail = typeof detailSource === 'string'
    ...
    : detailSource && typeof detailSource === 'object'
      ...
      : ''
  return `[SO Multi Agente hook] ${eventName}${tool}${detail ? `: ${detail.replace(/[\r\n]+/g, ' ').slice(0, 420)}` : ''}`
}
...
function agentInput(command: SandboxNode['command'], from: string, text: string): string {
  const message = `[SO Multi Agente message from ${from}] ${text.trim()}`
            ...
            const cleanup = await listenCodexAppServer(appServerId, (event) => {
              ...
              if (method === 'turn/completed') {
                ...
                if (reply && parent?.ptyId) {
                  const relay = `[SO Multi Agente reply from ${node.label}] ${reply}`
