# Adding a Theme to SO Multi Agente

Adding a custom theme is a great first contribution—it's visual, self-contained, and satisfying. In SO Multi Agente, a theme touches seven files/locations because the application UI and terminal palette are configured separately.
...
SO Multi Agente has both dark and light themes and is intended for long development sessions. Do not ship foreground/background combinations that are difficult to read.
